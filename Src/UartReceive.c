...
{
	if(uId <1 || uId>uUartIndex) return;
	Uart_Info* pUartInfo = pUartInfoArray[uId-1];
	HAL_UARTEx_ReceiveToIdle_DMA(pUartInfo->pHUart,pUartInfo->pReceive,UART_BUFFER_LENGTH);
}
...
{	
	...
	for(uint8_t index=0;index<uUartIndex;++index)
	{
		...
		if(pHUart == pUartInfo->pHUart)
		{
			...
			if(osOK == osMessageQueuePut(pUartInfo->hQueueId,&LOCAL_QUEUE_INFO,NULL,NULL))
			{
...
void ProcessUart(void)
{
	...
	for(uint8_t index=0;index<uUartIndex;++index)
	{
		...
		if(NULL != pUartInfo->pHUart)
		{
			...
			{
				/// 如果长度小于整个缓冲区得长度
				if((LOCAL_QUEUE_INFO.pBuffer - pUartInfo->pBuffer) + LOCAL_QUEUE_INFO.nLength <= UART_BUFFER_LENGTH)
				{
					memcpy(pLocalBuffer,LOCAL_QUEUE_INFO.pBuffer,LOCAL_QUEUE_INFO.nLength);
				}
				else
				{
					uint16_t nSubSize = (LOCAL_QUEUE_INFO.pBuffer - pUartInfo->pBuffer) + LOCAL_QUEUE_INFO.nLength - UART_BUFFER_LENGTH;
					uint16_t nPreSize = LOCAL_QUEUE_INFO.nLength - nSubSize;
					if(nPreSize>0) memcpy(pLocalBuffer,LOCAL_QUEUE_INFO.pBuffer,nPreSize);
					if(nSubSize>0) memcpy(pLocalBuffer+nPreSize,pUartInfo->pBuffer,nSubSize);
				}
				
				pUartInfo->pCallback(pUartInfo->pHUart,pLocalBuffer,LOCAL_QUEUE_INFO.nLength);
